...
def run_generative_eval(task_object, tokenizer, model, engine, num_samples, max_new_tokens, temperature, top_k, max_problems=None):
    ...
    for i in range(proc_idx, num_problems, num_devices):
        ...
        print(f"\r\033[KProcess {proc_idx} | {num_passed}/{total} ({100*num_passed/total:.2f}%)", end='', flush=True)
...
def run_categorical_eval(task_object, tokenizer, model, batch_size, max_problems=None):
    ...
    for i in range(proc_idx, num_batches, num_devices):
        ...
        # Forward pass — no grad in JAX by default
...
if __name__ == "__main__":
    ...
    parser.add_argument('-i', '--source', type=str, required=True, help="Source: sft|rl")
    parser.add_argument('-a', '--task-name', type=str, default=None, help="Task name (default=all, use | to split)")
    ...
    get_report().log(section="Chat evaluation " + args.source, data=[
        vars(args), results, chatcore_metric_dict,
    ])
