...
from nanochat.common import (compute_init, compute_cleanup, print0, DummyWandb,
                              print_banner, get_base_dir, get_peak_flops, get_device_name,
...
parser.add_argument("--window-pattern", type=str, default="SSSL", help="sliding window pattern: L=full, S=quarter context")
# Training horizon
parser.add_argument("--num-iterations", type=int, default=-1, help="explicit number of steps (-1 = auto)")
parser.add_argument("--target-flops", type=float, default=-1.0, help="target total FLOPs (-1 = disable)")
...
parser.add_argument("--embedding-lr", type=float, default=0.3, help="LR for embedding (AdamW)")
parser.add_argument("--unembedding-lr", type=float, default=0.008, help="LR for lm_head (AdamW)")
parser.add_argument("--weight-decay", type=float, default=0.28, help="weight decay for Muon")
parser.add_argument("--matrix-lr", type=float, default=0.02, help="LR for matrix params (Muon)")
parser.add_argument("--scalar-lr", type=float, default=0.5, help="LR for scalars (AdamW)")
parser.add_argument("--warmup-steps", type=int, default=40, help="LR warmup steps")
parser.add_argument("--warmdown-ratio", type=float, default=0.65, help="fraction of iterations for LR warmdown")
parser.add_argument("--final-lr-frac", type=float, default=0.05, help="final LR as fraction of peak")
parser.add_argument("--resume-from-step", type=int, default=-1, help="resume from checkpoint step")
# Evaluation
parser.add_argument("--eval-every", type=int, default=250, help="evaluate val loss every N steps (-1 = disable)")
parser.add_argument("--eval-tokens", type=int, default=80*524288, help="tokens for val evaluation")
...
parser.add_argument("--save-every", type=int, default=-1, help="checkpoint every N steps (-1 = only at end)")
# Output
parser.add_argument("--model-tag", type=str, default=None, help="override checkpoint directory name")
# Distributed (for multi-host TPU pods)
parser.add_argument("--coordinator-address", type=str, default=None, help="coordinator address for multi-host")
...
if resuming:
    ...
    model_state, opt_state_restored, meta_data = load_checkpoint(
        checkpoint_dir, args.resume_from_step, model
    )
...
if resuming and opt_state_restored is not None:
...
train_loader = tokenizing_distributed_data_loader_with_state_bos_bestfit(
    data_dir, tokenizer, args.device_batch_size, args.max_seq_len, repeat=True,
)
...
def train_step(model_state, opt_state, x, y):
    ...
    # Reconstruct model from state for nnx.value_and_grad
    graphdef = nnx.graphdef(model)

    def loss_fn(params):
        # Merge params back into model
        m = nnx.merge(graphdef, params)
        loss = m(x, y, loss_reduction='mean')
        return loss
...
# Get the model state as a pytree
...
while True:
    ...
    print0(f"step {step:05d}/{num_iterations:05d} ({pct_done:.2f}%) | loss: {debiased_smooth_loss:.6f} | dt: {dt * 1000:.2f}ms | tok/sec: {tok_per_sec:,} | bf16_mfu: {mfu:.2f}% | total time: {total_training_time/60:.2f}m{eta_str}")
